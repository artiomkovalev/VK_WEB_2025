...
class QuestionForm(BootstrapFormMixin, forms.ModelForm):
    ...
    def save(self, user=None, commit=True):
        with transaction.atomic():
            ...
            if tag_names:
                names = tag_names if isinstance(tag_names, list) else [t.strip() for t in tag_names.split(',') if t.strip()]
                if names:
...
class AnswerForm(BootstrapFormMixin, forms.ModelForm):
    class Meta:
        ...
        fields = ['text']
        widgets = {
            'text': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Write your answer here...'}),
        }
